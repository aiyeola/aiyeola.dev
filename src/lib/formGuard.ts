...
interface GuardOptions {
  /** Namespace for the rate-limit key, e.g. "subscribe". */
  scope: string;
...
export default async function guardFormRequest(
  ...
  if (Number.isFinite(elapsed) && elapsed < MIN_ELAPSED_MS) {
    console.warn(`[formGuard] ${scope}: submitted in ${elapsed}ms, dropping silently`);
    return { ok: false, silent: true };
  }

  const ip = getClientIp(req);

  const verified = await verifyTurnstile(body[TOKEN_FIELD], ip);
  if (!verified) {
    return {
      ok: false,
      status: 403,
  ...
  const { ok } = await rateLimit(`${scope}:${identifier}`, limit, windowSeconds);
  if (!ok) {
    return {
      ok: false,
      status: 429,
